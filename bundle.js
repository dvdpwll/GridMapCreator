...
/***/ function(module, exports, __webpack_require__) {
	...
	var grassImg = '<img class="layer0" data-layer="0" data-thing="grass" src="./assets/grass.png">'; //change this if you change the img file for grass.
	var rocksImg = '<img class="layer0" data-layer="0" data-thing="rocks" src="./assets/rocks.png">'; //change this if you change the img file for rocks.
	var waterImg = '<img class="layer0" data-layer="0" data-thing="water" src="./assets/water.png">'; //change this if you change the img file for water.
	var water2Img = '<img class="layer0" data-layer="0" data-thing="water2" src="./assets/water2.png">'; //change this if you change the img file for water2.
	var rockImg = '<img class="layer1" data-layer="1" data-thing="rock" src="./assets/rock.png">'; //change this if you change the img file for rock.
	var treeImg = '<img class="layer1" data-layer="1" data-thing="tree" src="./assets/tree.png">'; //change this if you change the img file for tree.
	var treesImg = '<img class="layer1" data-layer="1" data-thing="trees" src="./assets/trees.png">'; //change this if you change the img file for trees.
	...
	var LogInAfterSignUp = function LogInAfterSignUp(email, password) {
	...
	//send sign up data
	var onSignUpSubmit = function onSignUpSubmit() {
	  //get text fields
	  ...
	  //send data to api
	  api.signUp(data).done(LogInAfterSignUp(email, password)).fail(ui.failure);
	...
	//show change password modal
	var onChangePasswordSubmit = function onChangePasswordSubmit() {
	  //get text fields
	...
	//erase the grid
	var eraseGrid = function eraseGrid() {
	  $('.gameboard').empty();
	};

	//create new grid
	var createGrid = function createGrid(l, h) {
	...
	//place a thing
	var onMap = function onMap() {
	  //get the selected thing
	  var selected = $("input[name='thing']:checked").val();

	  //check if there's an img already in the square
	  var countImg = $(this).children().length;
	...
	//add grid handlers
	var addGridHandlers = function addGridHandlers() {
	...
	var newElements = function newElements(data) {
	  ...
	  var name = api.appVar.app.map.name;
	  var mapId = api.appVar.app.map.id;
	  var length = api.appVar.app.map.length;
	  var height = api.appVar.app.map.height;
	  $('.mapName').empty();
	  ...
	  $('.square').each(function () {
	    ...
	    api.newElement(data0).done(ui.newElementSuccess0).fail(ui.failure);

	    // send data to api, make an element for each square
	    api.newElement(data1).done(ui.newElementSuccess1).fail(ui.failure);
	  });
	};

	//on new map submit
	var onNewMapSubmit = function onNewMapSubmit() {
	  //get text fields
	...
	//send save data
	var sendSave = function sendSave(t, id, layer, count, totalElements) {
	  //put info in data object
	  var data = {
	    "element": {
	      "thing": t,
	  ...
	  api.saveElement(data, id).done(ui.saveElementsSuccess(count, totalElements)).fail(ui.failure);
	};

	//save current map
	var onSaveMap = function onSaveMap() {
	  //checks each square
	  $('.square').each(function () {
	    //get how many elements total
	    var totalElements = $('.square').length;
	    ...
	    //get id of element0
	    var id0 = api.appVar.app.elements0[order];

	    //get id of element1
	    var id1 = api.appVar.app.elements1[order];

	    //extracts the data-thing value from the imgs
	    var thing0 = $(this).find('.layer0').data('thing');
	    var thing1 = $(this).find('.layer1').data('thing');
	...
	var displayAllMaps = function displayAllMaps(data) {
	  //empty the body of the show all maps modal
	  $('#show-all-maps-body').empty();

	  //send data to handlebars
	  var listMapsTemplate = __webpack_require__(8);
	...
	var onSeeAllMaps = function onSeeAllMaps() {
	  //show the all maps modal
	  $('#show-all-maps-modal').modal('show');

	  //get all maps for user
	  api.seeAllMaps().done(displayAllMaps).fail(ui.failure);
	};

	//display one map
	var displayMap = function displayMap(data) {
	  ...
	  for (var i = 0; i < data.elements.length; i++) {
	    var order = data.elements[i].order;
	  ...
	  $('.square').each(function () {
	    //get index of square
	    var index = $(this).data('square');

	    //loop through elements array
	    for (var _i2 = 0; _i2 < data.elements.length; _i2++) {
	      //if element order # is same as square index number insert img
	      if (data.elements[_i2].order === index) {
	        var thing = data.elements[_i2].thing;
	...
	var onSelectMap = function onSelectMap(event) {
	  var target = $(event.target);
	  var mapId = target.data('id');
	...
	//on change map name
	var onChangeMapNameSubmit = function onChangeMapNameSubmit() {
	  //get text field
	  var name = $('#change-map-name-id').val();

	  //get map id
	  var id = api.appVar.app.map.id;
	  ...
	  //send data to api
	  api.patchMapName(data, id).done(ui.nameChangeSuccess(name)).fail(ui.failure);
	...
	//show load map modal
	var onLoadMapSubmit = function onLoadMapSubmit() {
	  //get text field
	...
	var onDeleteMap = function onDeleteMap() {
	  $('#confirm-delete-modal').modal('show');
	};

	//delete the map after finished deleteing all elements
	var deleteMap = function deleteMap() {
	  //get map id
	  var mapId = api.appVar.app.map.id;

	  //delete map by id
	  api.delMap(mapId).done(ui.deleteMapSuccess).fail(ui.failure);
	...
	var deleteElements = function deleteElements(data) {
	  //get map id
	  var mapId = api.appVar.app.map.id;
	...
	var onDeleteMapSubmit = function onDeleteMapSubmit() {
	  //get the mapId
	  var mapId = api.appVar.app.map.id;

	  //use map id to find all elements of a map
	  if (mapId) {
	    api.seeElements(mapId).done(deleteElements).fail(ui.failure);
	  }
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	//new map
	var newMap = function newMap(data) {
	  return $.ajax({
	...
	var newElement = function newElement(data) {
	  return $.ajax({
	...
	var seeAllMaps = function seeAllMaps() {
	  return $.ajax({
	...
	var seeElements = function seeElements(data) {
	  return $.ajax({
	...
	var saveElement = function saveElement(data, id) {
	  return $.ajax({
	...
	//delete an element
	var deleteElement = function deleteElement(m, e) {
	  return $.ajax({
	...
	var patchMapName = function patchMapName(data, id) {
	  return $.ajax({
	...
	module.exports = {
	  ...
	  seeAllMaps: seeAllMaps,
	  seeElements: seeElements,
	  saveElement: saveElement,
	  deleteElement: deleteElement,
	  delMap: delMap,
	  patchMapName: patchMapName,
	  appVar: appVar
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var newElementSuccess0 = function newElementSuccess0(data) {
	  //get order number
	  var index = data.element.order;
	...
	var newElementSuccess1 = function newElementSuccess1(data) {
	  //get order number
	  var index = data.element.order;
	...
	var nameChangeSuccess = function nameChangeSuccess(name) {
	  $('.mapName').empty();
	  $('.mapName').append("<h1>" + name + "</h1>");
	};

	//success for save elements
	var saveElementsSuccess = function saveElementsSuccess(count, totalElements) {
	...
	var passwordSuccess = function passwordSuccess() {
	  $('#messages-modal').modal('show');
	...
	var deleteMapSuccess = function deleteMapSuccess() {
	  $('.mapName').empty();
	...
	module.exports = {
	  ...
	  newElementSuccess0: newElementSuccess0,
	  newElementSuccess1: newElementSuccess1,
	  nameChangeSuccess: nameChangeSuccess,
	  saveElementsSuccess: saveElementsSuccess,
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
	  ...
	  return "<ul>\n"
	    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : {},(depth0 != null ? depth0.maps : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 0),"inverse":container.noop,"data":data})) != null ? stack1 : "")
	    + "</ul>\n";
	},"useData":true});
